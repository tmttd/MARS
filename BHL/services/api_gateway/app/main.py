...
async def conversion_webhook(job_id: str):
    try:
        ...
        async with httpx.AsyncClient() as client:
            conv_response = await client.get(f"{settings.CONVERTER_SERVICE_URL}/convert/{job_id}")
            if conv_response.status_code != 200:
                raise HTTPException(status_code=conv_response.status_code, detail="변환 상태 조회 실패")
            
            conv_data = conv_response.json()
            converted_file_path = conv_data.get("output_file")
            
            # 파일 경로를 api_gateway의 마운트된 경로로 변환
            converted_file_path = converted_file_path.replace("/app/audio_outputs", settings.AUDIO_OUTPUT_DIR)
            logger.info(f"변환된 파일 경로: {converted_file_path}")
            
            if not os.path.exists(converted_file_path):
                logger.error(f"파일이 존재하지 않음: {converted_file_path}")
                raise FileNotFoundError(f"변환된 파일을 찾을 수 없습니다: {converted_file_path}")
        ...
        raise HTTPException(status_code=e.response.status_code, detail="텍스트 변환 요청 실패")
